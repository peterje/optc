...
export const Operations: Component = () => {
  const { localStorageSettings: settings, mutateSettings, mutateLegends: mutate, localStorageLegends: lsl } = useRouteData<typeof routeData>();

  const import_share_code = (share_code: string) => {
    const decoded_legends = decode_legends(share_code)
    mutate(decoded_legends)
  }
  if (!settings.latest || !lsl.latest) return
  return (
    <div class="flex flex-col xl:flex-row justify-center gap-2 ">
      <button class="btn btn-outline btn-success" onclick={() => mutate(by(lsl(), () => true, l => ({ ...l, selected: true })))}>Select All</button>
      <button class="btn btn-outline btn-info" onclick={() => mutateSettings({ hideBaseForms: !settings.latest.hideBaseForms })}>{settings().hideBaseForms ? "Show" : "Hide"} ★6 Forms of Super-Evos</button>
      <button class="btn btn-outline btn-info" onclick={() => mutate(by(lsl(), l => l.removed_by_user, l => ({ ...l, removed_by_user: false })))}>Unhide All Removed Legends</button>
      ...
      <button class="btn btn-outline btn-info" onclick={() => { export_share_code(lsl()); toast.success("Saved share code to clipboard") }}>Export Share Code</button>
      <label for="share-code-modal" class="btn btn-outline btn-info">Import Share Code</label>
      <button class="btn btn-outline btn-error" onclick={() => mutate(by(lsl(), () => true, (l) => createLegend(l.id)))}>Reset All</button>
      ...
      <Modal id="removed-legends-modal">
        ...
        <div class='grid grid-cols-5 gap-4'>
          <For each={lsl().filter(l => l.removed_by_user)}>
            {(legend) => (<LegendIcon forceShow={true} legend={legend} onClick={() => mutate(by(lsl(), l => l.id === legend.id, l => ({ ...l, removed_by_user: false })))} />)}
          </For>
        </div>
        <div class="modal-action">
